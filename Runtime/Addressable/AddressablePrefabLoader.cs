...
namespace GGemCo.Scripts
{
    ...
    public class AddressablePrefabLoader : MonoBehaviour
    {
        ...
        private readonly Dictionary<string, GameObject> loadPrefabMonsters = new Dictionary<string, GameObject>();
        ...
        // 몬스터 프리팹 로드 후 생성
        public async Task LoadMonster()
        {
            try
            {
                loadPrefabMonsters.Clear();
                var locationHandle = Addressables.LoadResourceLocationsAsync("GGemCo_Character_Monster");
                ...
                foreach (var location in locationHandle.Result)
                {
                    ...
                    if (prefab != null)
                    {
                        loadPrefabMonsters[address] = prefab;
                        loadedCount++;
                    }
                }
