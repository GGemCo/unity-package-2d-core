...
{
    /// <summary>
    /// <see cref="Transform.localPosition"/> 기반 Move 유틸리티입니다.
    /// </summary>
    /// <remarks>
    /// - UI(<see cref="RectTransform"/>)가 아닌 일반 오브젝트의 로컬 이동에도 사용 가능합니다.
    /// - 동일 <see cref="Transform"/>에 대해 중복 Move 요청 시 기존 코루틴을 자동 중지합니다.
    /// - 지연/시간 스케일/이징/완료 스냅은 <see cref="MoveOptions"/>로 제어합니다.
    /// </remarks>
    public static class UiMoveLocalPosition
    {
        /// <summary>
        ...
        /// <summary>
        /// <paramref name="tr"/>를 지정한 로컬 위치(<see cref="Transform.localPosition"/>)로 이동시킵니다.
        /// </summary>
        /// <param name="runner">코루틴 실행자.</param>
        /// <param name="tr">이동 대상 Transform.</param>
        /// <param name="to">목표 로컬 위치.</param>
        ...
        public static Coroutine MoveTo(MonoBehaviour runner, Transform tr, Vector3 to, float duration, MoveOptions options)
        {
            ...
            state.running = runner.StartCoroutine(MoveToRoutine(tr, to, duration, options));
            return state.running;
        }

        /// <summary>
        /// 현재 로컬 위치를 기준으로 <paramref name="delta"/> 만큼 상대 이동시킵니다.
        /// </summary>
        /// <param name="runner">코루틴 실행자.</param>
        /// <param name="tr">이동 대상 Transform.</param>
        ...
        /// <returns>실행된 Move 코루틴. tr이 null이면 null.</returns>
        public static Coroutine MoveBy(MonoBehaviour runner, Transform tr, Vector3 delta, float duration, MoveOptions options)
        {
            if (tr == null) return null;
            return MoveTo(runner, tr, tr.localPosition + delta, duration, options);
        }
        ...
        public static IEnumerator MoveToRoutine(Transform tr, Vector3 to, float duration, MoveOptions options)
        {
            ...
            while (elapsed < duration)
            {
                ...
                tr.localPosition = Vector3.LerpUnclamped(from, to, et);
                yield return null;
            }
