...
namespace GGemCo2DCore
{
    ...
    /// HP 데미지와 속성 데미지 계산은 담당하지 않으며, 명시적으로 전달된 게이지 입력만 처리합니다.
    /// </remarks>
    ...
    public sealed class CharacterElementGaugeController : MonoBehaviour
    {
        ...
        private IElementGaugeRepeatedInputHandler _repeatedInputHandler = NullElementGaugeRepeatedInputHandler.Instance;
        ...
        /// <summary>
        /// 전역 기본 임계 상태 반복 입력 핸들러입니다.
        /// 프로젝트 부트스트랩에서 설정하면 이후 생성되는 컨트롤러의 기본 핸들러로 사용합니다.
        /// </summary>
        public static IElementGaugeRepeatedInputHandler DefaultRepeatedInputHandler { get; set; } = NullElementGaugeRepeatedInputHandler.Instance;
        ...
        /// <summary>
        /// 이미 임계 상태인 속성에 같은 속성 게이지 입력이 다시 들어왔을 때 임계 사이클당 한 번 발생합니다.
        /// </summary>
        public event Action<ElementGaugeSnapshot, ElementGaugeAccumulationContext> RepeatedGaugeInputReceived;
        ...
        private void Awake()
        {
            ...
            _repeatedInputHandler = DefaultRepeatedInputHandler ?? NullElementGaugeRepeatedInputHandler.Instance;
        ...
        /// <summary>
        /// 임계 상태 반복 입력 핸들러를 교체합니다.
        /// </summary>
        /// <param name="handler">프로젝트별 반복 입력 핸들러입니다. null이면 기본 Null 핸들러를 사용합니다.</param>
        public void SetRepeatedInputHandler(IElementGaugeRepeatedInputHandler handler)
        {
            _repeatedInputHandler = handler ?? NullElementGaugeRepeatedInputHandler.Instance;
        }
        ...
        /// <param name="gaugeAmount">속성 게이지에 더할 수치입니다.</param>
        ...
        /// <remarks>
        /// 이 메서드는 <see cref="CharacterBase.TakeDamage"/>를 호출하지 않으므로 HP 데미지, 피격 반응, Hit VFX, 넉백 처리를 발생시키지 않습니다.
        /// </remarks>
        public ElementGaugeAccumulationResult Accumulate(
            ...
            MetadataDamage metadataDamage = null)
        {
            ...
            if (!CanAccumulateElementGaugeType(damageType) || !CanAccumulateByPolicy(damageType))
                return ElementGaugeAccumulationResult.None;

            if (!_runtime.TryGetRule(damageType, out ElementGaugeRuleDefinition rule) || rule == null)
                return ElementGaugeAccumulationResult.None;

            return AccumulateResolvedAmount(metadataDamage, source, damageType, (long)Math.Ceiling(gaugeAmount), gaugeAmount);
        }
        ...
        /// <summary>
        /// 속성 게이지 누적 대상 타입인지 확인합니다.
        /// </summary>
        /// <param name="damageType">검사할 속성 타입입니다.</param>
        /// <returns>물리와 None이 아닌 속성이면 <see langword="true"/>입니다.</returns>
        private static bool CanAccumulateElementGaugeType(ConfigCommon.DamageType damageType)
        {
            ...
            float gaugeAmount)
        {
            ...
            ElementGaugeApplyResult result = _gaugeProcessor.AccumulateGauge(_runtime, damageType, gaugeAmount, Time.time);
            if (!result.GaugeChanged && !result.ThresholdReached && !result.RepeatedGaugeInput)
                return ElementGaugeAccumulationResult.None;
            ...
            else if (result.RepeatedGaugeInput)
            {
                RepeatedGaugeInputReceived?.Invoke(result.Snapshot, context);
                _repeatedInputHandler.OnRepeatedGaugeInput(result.Snapshot, context);
            }

            return new ElementGaugeAccumulationResult(
                result.GaugeChanged,
                result.ThresholdReached,
                result.RepeatedGaugeInput,
                result.Snapshot);
        }
