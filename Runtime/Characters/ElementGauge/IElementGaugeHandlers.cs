namespace GGemCo2DCore
{
    ...
    /// </summary>
    /// <remarks>
    /// Core는 어떤 효과를 발동할지 알지 않고, 프로젝트별 구현체가 이 인터페이스를 통해 Affect 적용, 추가 소모, 연출 등을 처리합니다.
    /// </remarks>
    ...
    /// <summary>
    /// 이미 임계 상태인 속성에 같은 속성 게이지 입력이 다시 들어왔을 때 임계 사이클당 한 번 호출되는 확장 핸들러입니다.
    /// </summary>
    public interface IElementGaugeRepeatedInputHandler
    {
        /// <summary>
        /// 임계 상태에서 같은 속성 게이지 입력을 처음 다시 받았을 때 호출됩니다.
        /// </summary>
        /// <param name="snapshot">현재 게이지 스냅샷입니다.</param>
        /// <param name="context">누적 원인과 대상 정보를 담은 컨텍스트입니다.</param>
        void OnRepeatedGaugeInput(ElementGaugeSnapshot snapshot, ElementGaugeAccumulationContext context);
    }
    ...
    /// </summary>
    public sealed class NullElementGaugeRepeatedInputHandler : IElementGaugeRepeatedInputHandler
    {
        public static readonly NullElementGaugeRepeatedInputHandler Instance = new NullElementGaugeRepeatedInputHandler();

        private NullElementGaugeRepeatedInputHandler()
        {
        }

        /// <inheritdoc />
        public void OnRepeatedGaugeInput(ElementGaugeSnapshot snapshot, ElementGaugeAccumulationContext context)
        {
        }
    }
}
