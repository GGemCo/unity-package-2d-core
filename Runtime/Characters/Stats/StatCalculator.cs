...
namespace GGemCo2DCore
{
    ...
    /// <remarks>
    /// 계산 규칙:
    /// - 시작값에 모든 Provider의 Flat(가산)을 먼저 합산한 뒤 Percent(%)를 적용합니다.
    /// - 시작값은 BASE_* 계산에서는 Base*, STAT_* 계산에서는 Stat* 값입니다.
    ...
    {
        /// <summary>
        /// 지정한 스탯 키에 대해 시작값과 Provider들의 modifier를 합산하여 최종값을 계산합니다.
        /// </summary>
        /// <param name="statKey">계산할 스탯 키(예: BASE_ATK, STAT_ATK 등)입니다.</param>
        /// <param name="startValue">계산 기준이 되는 시작값입니다. BASE_*는 Base*, STAT_*는 Stat* 값을 전달합니다.</param>
        /// <param name="providers">Flat/Percent modifier를 제공하는 Provider 목록입니다.</param>
        /// <returns>(시작값 + Flat합) * (1 + Percent합/100) 규칙으로 계산된 최종값입니다.</returns>
        ...
        private static long CalculateFinalInternal(int startValue, int flatBonus, float percentBonus)
        {
            ...
            // 기존 CharacterStat과 동일: (시작값 + flat) * multiplier 를 long으로 캐스팅(소수점은 절삭).
            return (long)((startValue + flatBonus) * finalMultiplier);
        }
    }
}