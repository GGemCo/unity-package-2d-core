...
namespace GGemCo2DCore
{
    ...
    public static class ConfigAddressableMap
    {
        ...
        // 패트롤 영역
        public static readonly AddressableAssetInfo ObjectPatrol = new(
            key: $"{ConfigDefine.NameSDK}_Map_Patrol",
            path: $"{ConfigAddressablePath.Maps.Common}/ObjectPatrol.prefab",
            label: ConfigAddressableLabel.PreLoadGamePrefabs
        );

        /// <summary>로딩 씬 프리로드 대상(읽기 전용)</summary>
        public static readonly ReadOnlyCollection<AddressableAssetInfo> NeedLoadInLoadingScene
            = new(new List<AddressableAssetInfo> { ObjectWarp, ObjectPatrol });
