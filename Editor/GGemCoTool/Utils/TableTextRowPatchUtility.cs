...
{
    /// <summary>
    /// 탭(	) 구분 텍스트 테이블 파일에서 특정 Uid 행만 교체/추가하는 공용 유틸리티입니다.
    /// - 실제 파일의 헤더 순서를 기준으로 직렬화합니다.
    /// - 주석/다른 행/정렬은 최대한 그대로 유지합니다.
    /// - 대상 Uid 행이 없으면 파일 끝에 append 합니다.
    /// </summary>
    public static class TableTextRowPatchUtility
    {
            ...
            out string error)
        {
            ...
            try
            {
                ...
                int uidColumnIndex = Array.FindIndex(headers, static h => string.Equals(h?.Trim(), "Uid", StringComparison.Ordinal));
                if (uidColumnIndex < 0)
                {
                    error = $"Uid 헤더를 찾을 수 없습니다: {assetPath}";
                    return false;
                }
                ...
                {
                    error = $"직렬화된 Row가 비어있습니다. uid={uid}";
                    return false;
                }
                ...
                for (int i = 1; i < lines.Count; i++)
                {
                    ...
                    if (values.Length <= uidColumnIndex)
                        continue;

                    if (!int.TryParse(values[uidColumnIndex], out int rowUid))
                        continue;
