...
{
    public class SettingWorldMap : DefaultAddressable
    {
        private const string Title = "월드맵 JSON 추가하기";
        private readonly AddressableEditor _addressableEditor;
        
        public SettingWorldMap(AddressableEditor addressableEditorWindow)
        {
            _addressableEditor = addressableEditorWindow;
            targetGroupName = ConfigAddressableGroupName.WorldMap;
        }
        public void OnGUI()
        {
            if (!File.Exists($"{ConfigAddressableWorldMap.GetAssetPath("")}"))
            {
                EditorGUILayout.HelpBox($"{ConfigAddressableWorldMap.GetFileName("")} 파일이 없습니다.", MessageType.Info);
            }
            else
            {
        ...
        public void Setup(EditorSetupContext ctx = null)
        {
            ...
            if (ctx != null)
            {
                HelperLog.Info("[Addressable] 월드맵 설정 완료", ctx);
            }
            else
            {
                AssetDatabase.SaveAssets();
                EditorUtility.DisplayDialog(Title, "[Addressable] 월드맵 설정 완료", "OK");
            }
        }
    }
}