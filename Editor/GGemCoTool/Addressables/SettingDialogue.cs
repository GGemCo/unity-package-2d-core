...
namespace GGemCo2DCoreEditor
{
    public class SettingDialogue : DefaultAddressable
    {
        ...
        public void OnGUI()
        {
            if (!File.Exists($"{ConfigAddressableTable.TableDialogue.Path}"))
            {
            ...
            else
            {
                if (GUILayout.Button(Title, GUILayout.Width(_addressableEditor.buttonWidth), GUILayout.Height(_addressableEditor.buttonHeight)))
                {
                    ...
                    catch (System.Exception e)
                    {
                        ...
                        EditorUtility.DisplayDialog(Title, "대화 Addressable 설정 중 오류가 발생했습니다.\n자세한 내용은 콘솔 로그를 확인해주세요.", "OK");
                    }
                }
            }
        }
        ...
        public void Setup(EditorSetupContext ctx = null)
        {
            ...
            Dictionary<int, StruckTableDialogue> dictionary = TableLoaderManager.LoadDialogueTable().GetDatas();
            ...
            foreach (KeyValuePair<int, StruckTableDialogue> outerPair in dictionary)
            {
            ...
            if (ctx != null)
            {
                HelperLog.Info("[Addressable] 대화 설정 완료", ctx);
            }
            else
            {
                AssetDatabase.SaveAssets();
                EditorUtility.DisplayDialog(Title, "[Addressable] 대화 설정 완료", "OK");
            }
        }
    }
}